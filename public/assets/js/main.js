var Amber = {
    ...
    initColorPicker: function () {
        var body = $('body');
        $.cookie("old-theme", "theme-4");
        var colorSwatch = $('.color-swatch');
        colorSwatch.on('click', function (event) {
            if ($(this).hasClass('theme-1')) {
                $.cookie("theme", "theme-1");
                ga('send', {
                    'hitType': 'event',          // Required.
                    'eventCategory': 'button',   // Required.
                    'eventAction': 'click',      // Required.
                    'eventLabel': 'Select color',
                    'eventValue': 'purple'
                });
            } else if ($(this).hasClass('theme-2')) {
                $.cookie("theme", "theme-2");
                ga('send', {
                    'hitType': 'event',          // Required.
                    'eventCategory': 'button',   // Required.
                    'eventAction': 'click',      // Required.
            ...
            } else if ($(this).hasClass('theme-3')) {
                $.cookie("theme", "theme-3");
                ga('send', {
                    'hitType': 'event',          // Required.
                    'eventCategory': 'button',   // Required.
                    'eventAction': 'click',      // Required.
            ...
            } else if ($(this).hasClass('theme-4')) {
                $.cookie("theme", "theme-4");
                ga('send', {
                    'hitType': 'event',          // Required.
                    'eventCategory': 'button',   // Required.
                    'eventAction': 'click',      // Required.
            ...
            } else if ($(this).hasClass('theme-5')) {
                $.cookie("theme", "theme-5");
                ga('send', {
                    'hitType': 'event',          // Required.
                    'eventCategory': 'button',   // Required.
                    'eventAction': 'click',      // Required.
            ...
            if ($.cookie("theme")) {
                body.removeClass($.cookie("old-theme"));
                body.addClass($.cookie("theme"));
                colorSwatch.removeClass('active');
                $('.color-swatch.' + $.cookie("theme")).addClass('active');
                $.cookie("old-theme", $.cookie("theme"));
            }
            event.preventDefault();
        });
        if ($.cookie("theme")) {
    ...
    initPortfolio: function () {
        ...
        $container.find('article').find('.overlay').each(function () {
            $(this).hoverdir({
                hoverDelay: 75
            });
        });
    ...
    initElementTransitions: function () {
        if ($(window).width() >= 786) {
            ...
            $('h1').css('opacity', 0).bind('inview', function (isInView) {
                if (isInView) {
                    $(this).addClass('animated fadeInUp delayed-1');
                }
            });
            ...
            $('.services-summary').css('opacity', 0).bind('inview', function (isInView) {
                if (isInView) {
                    $(this).addClass('animated fadeInUp delayed-2');
                }
            });
            ...
            $('.description').css('opacity', 0).bind('inview', function (isInView) {
                if (isInView) {
                    $(this).addClass('animated fadeInUp delayed-3');
                }
            });
            ...
            pricingBox.find('.price').bind('inview', function (isInView) {
                if (isInView) {
                    $(this).addClass('animated pulse delayed-2');
                }
            });
...
$(document).ready(function () {
    ...
    $('input[name=cant]').on('blur',function (e) {
        let val = $(this).val();
        //console.log(val)
    });
});
